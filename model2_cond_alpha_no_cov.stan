...
model {
  ...
  for (i in 1:I) {
    for (k in 1:K) {
      ...
      // P_nu=1 so X_nu is just [1] --> intercept only, no covariate
      real mu_alpha_ik = X_alpha[i, k] * beta_alpha + v[i] * X_alpha[i, k][2];
...
// save predicted means per participant x condition for design matrix checking
generated quantities {
  ...
  for (i in 1:I) {
    for (k in 1:K) {
      ...
      mean_alpha_ik[i, k] = X_alpha[i, k] * beta_alpha + v[i] * X_alpha[i, k][2];
