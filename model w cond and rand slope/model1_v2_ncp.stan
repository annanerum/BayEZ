// model: alpha=cond+slope, nu=cov, tau=intercept
// v[i] = beta_alpha[2] + sigma_v * z_v[i]
// v[i] is the participant's total condition effect on alpha
// centred at the population mean beta_alpha[2], not 0
// shrinkage pulls toward beta_alpha[2] 

data {
  int I;
  int K;
  int P_nu;     // 2: intercept + covariate
  int P_alpha;  // 2: intercept + condition mean
...
parameters {
  vector[P_nu]    beta_nu;
  // beta_alpha[1] = population mean alpha intercept
  // beta_alpha[2] = population mean condition effect (prior mean for v[i])
  ...
  vector[I] z_v;  // non-centred raw slopes
...
transformed parameters {
  // v[i] is the participant's total condition effect
  // centred at beta_alpha[2] (population mean), SD = sigma_v
  vector[I] v = beta_alpha[2] + sigma_v * z_v;
}
...
model {
  ...
  for (i in 1:I) {
    for (k in 1:K) {
      ...
      // v[i] is now the slope, beta_alpha[2] is absorbed into v[i]
      // intercept part: beta_alpha[1] * 1
      // condition part: v[i] * cond_dummy (v[i] already contains population mean)
      real mu_alpha_ik = beta_alpha[1] + v[i] * X_alpha[i, k][2];
...
generated quantities {
  ...
  for (i in 1:I) {
    for (k in 1:K) {
      ...
      mean_alpha_ik[i, k] = beta_alpha[1] + v[i] * X_alpha[i, k][2];
