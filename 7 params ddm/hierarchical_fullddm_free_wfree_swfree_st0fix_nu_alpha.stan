...
functions {
  real partial_sum(array[] int idx_slice, int start, int end,
                    ...
                    vector t0,
                    real w, real sv, real sw, vector st0) {
    real lp = 0;
    for (n in 1:size(idx_slice)) {
      ...
      if (acc[i] == 1) {
        lp += wiener_lpdf(rt[i] | a, t0[p], w, v, sv, sw, st0[p]);
      } else {
        lp += wiener_lpdf(rt[i] | a, t0[p], 1 - w, -v, sv, sw, st0[p]);
      }
    }
    return lp;
  }
...
transformed parameters {
  ...
  real sw = sw_raw * 2 * fmin(w, 1 - w);   // unchanged -- smooth, single-scalar bound

  // per-participant bound, no min() across participants (the fix)
  vector[I] st0 = st0_raw * 2 * t0;
}
