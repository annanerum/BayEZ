...
//   st0: free, single population value (st0_raw * 2*min(t0))
...
// EXPECT THIS TO BE EXPENSIVE -- both sw and st0 trigger numerical
// integration in wiener_lpdf, confirmed ~7000x slower than the zero model
// for st0 alone; having both free simultaneously will likely be worse still
...
functions {
  real partial_sum(array[] int idx_slice, int start, int end,
                    ...
                    vector t0,
                    real w, real sv, real sw, real st0) {
    real lp = 0;
    for (n in 1:size(idx_slice)) {
      ...
      if (acc[i] == 1) {
        lp += wiener_lpdf(rt[i] | a, t0[p], w, v, sv, sw, st0);
      } else {
        lp += wiener_lpdf(rt[i] | a, t0[p], 1 - w, -v, sv, sw, st0);
      }
    }
    return lp;
  }
...
data {
  ...
  vector[2] prior_w;
  vector[2] prior_sv;
  vector[2] prior_sw;    // prior on sw_raw
...
parameters {
  ...
  real<lower=0>           sv;
  real<lower=0, upper=1>  sw_raw;
  real<lower=0, upper=1>  st0_raw;
}
...
model {
  ...
  w       ~ normal(prior_w[1],   prior_w[2]);
  sv      ~ normal(prior_sv[1],  prior_sv[2]);
  sw_raw  ~ normal(prior_sw[1],  prior_sw[2]);
