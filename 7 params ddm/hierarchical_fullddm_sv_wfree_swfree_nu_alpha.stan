...
//   sw : free, single population value. Needs sw_raw reparameterization
//        (sw = sw_raw * 2*min(w,1-w)) since the valid range depends on w,
//        which is no longer fixed at 0.5 here.
...
functions {
  real partial_sum(array[] int idx_slice, int start, int end,
    ...
    for (n in 1:size(idx_slice)) {
      ...
      if (acc[i] == 1) {
        lp += wiener_lpdf(rt[i] | a, t0[p], w, v, sv, sw, 0.0);
      } else {
        lp += wiener_lpdf(rt[i] | a, t0[p], 1 - w, -v, sv, sw, 0.0);
      }
    }
    return lp;
  }
