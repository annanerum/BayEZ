...
//   sw : fixed at 0
//   sv : RANDOM per participant (log scale for positivity), not a single
//        population value -- each person has their own level of
//        trial-to-trial drift noise: sv[i] = exp(mu_sv + sigma_sv*z_sv[i])
//   st0: fixed at 0
// sv is still resolved analytically in wiener_lpdf regardless of its value,
// so this should stay computationally cheap -- the risk here is
// identifiability (each sv[i] estimated from only that person's own
// trials), not raw cost
...
transformed parameters {
  ...
  vector[I] sv = exp(sv_intercept_log);  // positivity guaranteed
}
