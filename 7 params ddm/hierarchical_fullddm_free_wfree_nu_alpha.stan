...
// EXPECT THIS TO BE EXPENSIVE like the original "free" model -- st0 alone
// triggers numerical integration regardless of w
...
functions {
  real partial_sum(array[] int idx_slice, int start, int end,
    ...
    for (n in 1:size(idx_slice)) {
      ...
      if (acc[i] == 1) {
        lp += wiener_lpdf(rt[i] | a, t0[p], w, v, sv, 0.0, st0);
      } else {
        lp += wiener_lpdf(rt[i] | a, t0[p], 1 - w, -v, sv, 0.0, st0);
      }
    }
    return lp;
  }
