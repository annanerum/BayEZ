...
//   sv : fixed at 0
//   st0: fixed at 0
// EXPECT THIS TO BE EXPENSIVE like the "free" model -- sw alone triggers
// numerical integration in wiener_lpdf, same mechanism as st0
...
functions {
  real partial_sum(array[] int idx_slice, int start, int end,
    ...
    for (n in 1:size(idx_slice)) {
      ...
      if (acc[i] == 1) {
        lp += wiener_lpdf(rt[i] | a, t0[p], w, v, 0.0, sw, 0.0);
      } else {
        lp += wiener_lpdf(rt[i] | a, t0[p], 1 - w, -v, 0.0, sw, 0.0);
      }
    }
    return lp;
  }
