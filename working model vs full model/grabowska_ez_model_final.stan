...
// alpha	Resp_type (r)	Fixed (alpha_r[i]) !!
...
data {
  ...
  vector[2] prior_sigma_nu_intercept;
...
parameters {
  ...
  // differnet than sigma_nu / sigma_alpha / sigma_tau which are
  // the cell-level residual sds
...
transformed parameters {
  ...
  // vector[I] nu_intercept    = beta_nu[1]    + sigma_nu_intercept    * z_nu_intercept;
  vector[I] alpha_intercept = beta_alpha[1] + sigma_alpha_intercept * z_alpha_intercept;
  vector[I] tau_intercept   = beta_tau      + sigma_tau_intercept   * z_tau_intercept; // beta_tau no [1] index
...
model {
  ...
  // sigma_alpha_intercept: changed to gamma() to match Grabowska's sigma_theta(alpha) ~ Gamma(1,1)
  sigma_alpha_intercept ~ gamma(prior_sigma_alpha_intercept[1], prior_sigma_alpha_intercept[2]);
  // sigma_tau_intercept: changed to gamma() to match Grabowska's sigma_theta(tau) ~ Gamma(0.3,1)
  ...
  // priors for the intercept z's 
  // z_nu_intercept    ~ normal(0, 1);
  z_alpha_intercept ~ normal(0, 1);
  z_tau_intercept   ~ normal(0, 1);

  // parameters in every cell and EZ likelihood 
  for (i in 1:I) {
    for (k in 1:K) {
      ...
      // intercept (person-specific) + resp_type (FIXED, was random) + condition (fixed)
      real mu_alpha_ik = alpha_intercept[i] * X_alpha[i, k][1]
                       + beta_alpha[2]       * X_alpha[i, k][2]  
                       + beta_alpha[3]       * X_alpha[i, k][3];

      // tau: same across cells per person
     real mu_tau_ik = tau_intercept[i];
