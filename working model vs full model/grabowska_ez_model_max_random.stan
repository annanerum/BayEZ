...
transformed parameters {
  ...
  vector[I] alpha_c = beta_alpha[3] + sigma_alpha_c * z_alpha_c;   
}
...
model {
  ...
  sigma_alpha_c ~ normal(prior_sigma_alpha_c[1], prior_sigma_alpha_c[2]);   
  ...
  for (i in 1:I) {
    for (k in 1:K) {
      // linear predictor for nu: intercept now person-specific again (NEW)
      real mu_nu_ik = nu_intercept[i] * X_nu[i, k][1]
                    + nu_c[i]         * X_nu[i, k][2]
                    + nu_r[i]         * X_nu[i, k][3]
                    + nu_cr[i]        * X_nu[i, k][4];

      // linear predictor for alpha: condition effect now person specific too 
      real mu_alpha_ik = alpha_intercept[i] * X_alpha[i, k][1]
                       + alpha_r[i]          * X_alpha[i, k][2]
                       + alpha_c[i]          * X_alpha[i, k][3];
