...
parameters {
  // group-level means
  real<lower=0.1, upper=3.0> mu_alpha;
  real<lower=-3, upper=3> mu_nu;
  real<lower=0.05> mu_tau;

  // group-level SDs
  real<lower=0.1, upper=0.4> sigma_alpha;
  real<lower=0.2, upper=0.4> sigma_nu;
  real<lower=0.05, upper=0.25> sigma_tau;
...
model {
  ...
  sigma_alpha ~ uniform(0.1, 0.4);
  sigma_nu    ~ uniform(0.2, 0.4);
  sigma_tau   ~ uniform(0.05, 0.25);

  delta ~ uniform(0, 1);
  // perspectivisch normalverteilung mit mittelwert 0 und sd 0.1 
