...
priors <- list(
  ...
  # NEW: between-person random-INTERCEPT SDs.
  # Paper (Appendix C) puts the intercept SD (sigma_theta(delta)/sigma_theta(alpha))
  # under the SAME Gamma(1,1) prior as the condition-slope SD -- so we reuse the
  # same approximation as sigma_nu/sigma_alpha/sigma_tau above.
  prior_sigma_nu0    = c(0.3,  0.3),
  prior_sigma_alpha0 = c(0.3,  0.3),
  prior_sigma_tau0   = c(0.1,  0.15),
...
GROUP_PARS <- c(
  ...
  "sigma_nu0", "sigma_alpha0", "sigma_tau0",        # NEW: intercept SDs
...
fit <- sampling(
  ...
  chains = 5, iter = 2000, warmup = 100, # chains = 6/4, iter = 2000/1500, warmup = 1000/750,
...
if (rhat_max > 1.05) {
  ...
  s        <- summary(fit)$summary
  rhat_max <- max(s[GROUP_PARS, "Rhat"], na.rm = TRUE)
  cat(sprintf("Max Rhat after refit: %.3f\n", rhat_max))
}
...
# forest plot
# jeder Punkt ist der posterior mean of one population effect
# balken =  95%-Credible-Interval
# line at 0 = reference line 
...
# vertikale Linien = je eine Person
# von niedrigster Slope (unten) nach höchster Slope (oben)
# goldener Strich = population mean
# white line = 0 
...
# extract posterior summary for a vector of stan param names
...
# same as before but 50% CI (thick bar) and 95% CI (thin bar)
...
# add parameter type for colouring
...
p_forest_group <- ggplot(grp_df,
  ...
  labs(title = "Population effects — Krakow EZ-DDM",
...
p_forest_sds <- ggplot(sd_df,
  ...
  geom_errorbarh(aes(xmin = lo50, xmax = hi50), height = 0, linewidth = 2.5, alpha = 0.8) +
  geom_point(size = 3) +
  scale_colour_manual(values = c(residual = "goldenrod", slope = "steelblue", intercept = "#e07070")) +
  labs(title = "Between-person SDs ",
       subtitle = "Thick bar = 50% CI, thin bar = 95% CI",
...
# caterpillar plot: individual slopes (sorted, 95% CI)
# each line is a person's estimated congruency effect on drift rate
...
# extract population means from summary
...
# each violin is the distribution of the participants' posterior mean drift rates for that cell
# with individual dots and white diamond as cell mean
...
cat("Files:\n")
cat(paste0("  ", list.files(OUT, pattern="\\.png$"), collapse="\n"), "\n")
