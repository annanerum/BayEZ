...
priors <- list(
  ...
  # beta_alpha: [intercept, response_type]
  # intercept ~ N(1,1); resp ~ N(0,0.2)
...
for (i in seq_len(I)) for (k in seq_len(K)) {
  ...
  X_alpha[i, k, ] <- CELL_DESIGN[k, c(1, 3)]   # [1, resp]
...
sd_df <- extract_summary(sd_pars, sd_labels) |>
  mutate(type = c("residual","residual","residual","slope","slope","slope","slope"))
