...
OUT       <- "output_krakow_intercepts"
STAN_FILE <- "grabowska_ez_model_ppn_intercept.stan"  
...
priors <- list(
  ...
  # new: random intercept SDs (between-person variation in intercepts)
  # own creation, maybe do sanity check aka prior predictives or so
...
fit <- sampling(
  ...
  chains = 5, iter = 2000, warmup = 1000, # chains = 6/4, iter = 2000/1500, warmup = 1000/750,
...
sd_df <- extract_summary(sd_pars, sd_labels) |>
  mutate(type = c("residual","residual","residual",
                   ...
                   "intercept","intercept","intercept"))  # NEW type
...
# new: caterpillar plots for the participant-specific intercepts
