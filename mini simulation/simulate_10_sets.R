...
run_one_rep <- function(rep_id, seed) {
  ...
  subj_par$min_rt <- pmax(rnorm(n_subj, mean = 0.35, sd = 0.03), 0.25)
  subj_par$t0     <- subj_par$min_rt * plogis(subj_par$tau_intercept_raw)
  ...
  cat(sprintf("rep%02d done | seed=%d | trials=%d | mean_rt=%.3f | acc=%.3f\n",
              rep_id, seed, nrow(sim_data), mean(sim_data$rt), mean(sim_data$acc == 1)))
...
write.csv(subj_par[, c("subj", "min_rt", "t0", "st0")],
