...
functions {
  real partial_sum(array[] int idx_slice, int start, int end,
    ...
    for (n in 1:size(idx_slice)) {
      ...
      if (acc[i] == 1) {
        lp += wiener_lpdf(rt[i] | a, t0[p], w, v, sv[p], 0.0, 0.0);
      } else {
        lp += wiener_lpdf(rt[i] | a, t0[p], 1 - w, -v, sv[p], 0.0, 0.0);
      }
    }
    return lp;
  }
...
data {
  ...
  vector[2] prior_w;
  vector[2] prior_beta_sv;    // NEW: prior on population-mean log(sv)
  vector[2] prior_sigma_sv;   // NEW: prior on between-subject SD of log(sv)
}
transformed data {
  array[N] int idx;
...
parameters {
  ...
  // hierarchical sv, non-centered, log link keeps sv > 0 with no truncation
  real beta_sv;
...
transformed parameters {
  ...
  vector[I] sv = exp(beta_sv + sigma_sv * z_sv);   // per-subject sv
}
model {
  ...
  // hierarchical sv priors
  beta_sv  ~ normal(prior_beta_sv[1],  prior_beta_sv[2]);
  sigma_sv ~ gamma(prior_sigma_sv[1],  prior_sigma_sv[2]);
  z_sv     ~ normal(0, 1);
