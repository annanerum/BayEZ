...
true_pop <- list(
  beta_nu1 = 1.10, nu_c = 0.7, nu_r = 0.45, nu_cr = 0.25,
  beta_alpha1 = log(1.8), beta_alpha2 = 0.024, beta_alpha3 = 0.05,
  beta_tau = 0.30,
  w = 0.52, sw = 0.15, sv = 0.50, st0_raw = 0.15
)
...
run_one_rep <- function(rep_id, seed) {
  ...
  subj_par$t0  <- global_min_rt * plogis(subj_par$tau_intercept_raw)
  ...
  if (frac_t0_above_min_rt > 0) {
    warning(sprintf("rep%02d: t0 exceeds min_rt_observed for %.1f%% of subjects --> global_min_rt too high",
  ...
  saveRDS(global_min_rt, file.path(out_dir, "global_min_rt.rds"))
  
  cat(sprintf("rep%02d done | seed=%d | mean_rt=%.3f | mean_t0=%.3f | acc=%.3f | frac_t0_above_min_rt=%.3f\n",
              rep_id, seed, mean(sim_data$rt), mean(subj_par$t0), mean(sim_data$acc == 1), frac_t0_above_min_rt))
  invisible(sim_data)
}
