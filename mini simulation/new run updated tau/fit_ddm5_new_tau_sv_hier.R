...
m2sv_file <- "stan/M2_sv_hier_globalminrt.stan"
...
make_init_fun <- function(I) {
  function() {
    list(
      ...
      sigma_tau_intercept   = 0.1 + runif(1, 0, 0.05),
...
build_stan_data <- function(rep_id) {
  ...
  stan_data_base <- list(
    ...
    global_min_rt = global_min_rt, grainsize = 1,
    ...
    prior_sigma_alpha_intercept = c(1, 3), prior_sigma_tau_intercept = c(1, 20)
  )
