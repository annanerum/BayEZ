...
data {
  ...
  vector[N] condition;
  vector[N] resp_type;
  real<lower=0> global_min_rt;              // fixed now
  ...
  vector[2] prior_w;
  vector[2] prior_beta_sv;
  vector[2] prior_sigma_sv;
}
transformed data {
  array[N] int idx;
...
transformed parameters {
  ...
  vector[I] t0 = global_min_rt * inv_logit(tau_intercept_raw);   
  vector[I] sv = exp(beta_sv + sigma_sv * z_sv);
}
model {
