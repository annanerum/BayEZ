---
title: "simulation with condition matrices and random slope"
...
# ── Design ─────────────────────────────────────────────────────────────────────
I_vals <- c(20, 60, 100) #150 #320
J_vals <- c(20, 60, 100) #150 #320
N_sim  <- 10 #50
K      <- 2
...
derive_priors <- function(ez) {
  ...
  nu_sd    <- max(sd(nu_vals),    0.3)
  alpha_sd <- max(sd(alpha_vals), 0.2)
  tau_sd   <- max(sd(tau_vals),   0.1)
...
run_one <- function(I, J_per, true, model,
  ...
  for (i in seq_len(I)) {
    for (k in seq_len(K)) {
      ...
      VRT_mat[i, k] <- max(rnorm(1, sig2rt,
                                 sqrt(2 * sig2rt^2 / max(C_ik - 1, 1))), 1e-6)
      C_mat[i, k]   <- C_ik
    }
  }

  ez  <- ez_point_estimates(C_mat, J_mat, MRT_mat, VRT_mat, I, K)
  prs <- derive_priors(ez)
  ...
  # ── Group-level parameters only for convergence checks ──────────────────────
  # Using only these prevents large-I fits being wrongly excluded because
  # a single individual-level parameter briefly has a bad Rhat.
  group_pars <- c(
  ...
  hopeless <- function(fit) {
    rh <- tryCatch(
      summary(fit)$summary[group_pars, "Rhat"],
  ...
  indiv_results <- data.frame(
    ...
    est_alpha  = as.vector(alpha_est),
    true_nu    = as.vector(nu_true),
    est_nu     = as.vector(nu_est),
    true_tau   = as.vector(tau_true),
    est_tau    = as.vector(tau_est)
  )
...
clusterExport(cl, varlist = c("run_one", "ez_point_estimates", "derive_priors", "true"))
...
results_list <- parLapply(cl, jobs_list, function(row) {
  run_one(I = row$I, J_per = row$J, true = true, model = ez_cond)
})

stopCluster(cl)

saveRDS(results_list, file = "simulation_results.rds")
...
exclusion_report <- data.frame(
  ...
  Excluded       = n_excluded,
  Error          = n_error,
  `Excluded (%)` = round(100 * n_excluded / n_total, 1),
  check.names    = FALSE
)
...
results_indiv <- results_indiv %>%
  add_labels() %>%
  mutate(condition = factor(condition, labels = c("Easy (k=1)", "Hard (k=2)")))
...
# — Violin bias plots (group-level parameters) —
...
p_mu_nu    <- plot_group_bias("mu_nu",    expression(mu^(nu)))
p_mu_alpha <- plot_group_bias("mu_alpha", expression(mu^(alpha)))
p_mu_tau   <- plot_group_bias("mu_tau",   expression(mu^(tau)))
p_b1_nu    <- plot_group_bias("b1_nu",    expression(beta[1]^(nu)~"(covariate)"))
p_b2_nu    <- plot_group_bias("b2_nu",    expression(beta[2]^(nu)~"(condition)"))
...
# — Individual-level recovery plots: true vs. estimated, by condition —
plot_indiv_recovery <- function(true_col, est_col, label) {
  ggplot(results_indiv, aes_string(x = true_col, y = est_col, colour = "condition")) +
    ...
    scale_colour_manual(values = c("Easy (k=1)" = "goldenrod",
                                   "Hard (k=2)" = "steelblue")) +
    facet_grid(I_label ~ J_label) +
    labs(title = label,
         x     = paste("True", label),
         y     = paste("Estimated", label),
         colour = "Condition") +
    dark_theme +
    theme(legend.position = "bottom")
}

p_indiv_alpha <- plot_indiv_recovery("true_alpha", "est_alpha",
                                     expression(alpha[ik]))
p_indiv_nu    <- plot_indiv_recovery("true_nu",    "est_nu",
                                     expression(nu[ik]))
p_indiv_tau   <- plot_indiv_recovery("true_tau",   "est_tau",
                                     expression(tau[ik]))

for (p in list(p_indiv_alpha, p_indiv_nu, p_indiv_tau)) print(p)

# — Random slope recovery: true v_i vs. estimated v_i —
p_v_recovery <- ggplot(results_v, aes(x = true_v, y = est_v)) +
  ...
  facet_grid(I_label ~ J_label) +
  labs(
    title = expression("Recovery of by-participant random slopes " * v[i]),
...
prior_summary <- results_group %>%
  group_by(param, I, J) %>%
  summarise(
    `Prior mean (avg)` = round(mean(prior_mean, na.rm = TRUE), 3),
...
kable(prior_summary,
      caption = "Average data-derived prior hyperparameters across simulations,
                 by parameter, number of participants (I), and trials per condition (J).")
```
...
save_plot <- function(plot, filename, width = 8, height = 6) {
  ggsave(file.path("output", filename), plot,
...
save_plot(p_mu_nu,    "bias_mu_nu.png")
save_plot(p_mu_alpha, "bias_mu_alpha.png")
save_plot(p_mu_tau,   "bias_mu_tau.png")
save_plot(p_b1_nu,    "bias_b1_nu.png")
save_plot(p_b2_nu,    "bias_b2_nu.png")
save_plot(p_b_alpha,  "bias_b_alpha.png")
save_plot(p_b_tau,    "bias_b_tau.png")
save_plot(p_sigma_v,  "bias_sigma_v.png")
...
kable(prior_summary,
      caption = "Average data-derived priors (on hyperparameters) across simulations,
                 by parameter, number of participants, and trials per condition")
```

```{r third try with bug fixes}
library(rstan)
library(bayesplot)
library(ggplot2)
library(dplyr)
...
true <- list(
  mu_nu    = 0.0, 
  b1_nu    = 0.3,    # log-scale covariate effect one SD in x multiplies drift by exp(0.3) 
  b2_nu    = -0.5,   # log-scale condition effect condition 2 drift exp(-0.5) × condition 1
  mu_alpha = 1.5,
  b_alpha  = 0.2,
  mu_tau   = 0.3,
  b_tau    = 0.15,
  sigma_nu    = 0.3,   # SD on the log scale
  sigma_alpha = 0.25,
  sigma_tau   = 0.15,
  sigma_v     = 0.2
)
...
# prior derivation from EZ estimates
derive_priors <- function(ez) {
  ...
  nu_sd    <- min(max(sd(log_nu_vals), 0.3), 1.5)  # SD on log scale, bounded
  alpha_sd <- min(max(sd(alpha_vals),  0.2), 1.0)
  tau_sd   <- min(max(sd(tau_vals),    0.1), 0.5)
  ...
  list(
    ...
    beta_tau   = matrix(c(mean(tau_vals),   0,
                          tau_sd * 2,       tau_sd * 2),
                        nrow = 2, byrow = TRUE),
    sigma_nu    = c(nu_sd,    nu_sd / 2),   # log-scale SD prior
    sigma_alpha = c(alpha_sd, alpha_sd / 2),
    sigma_tau   = c(tau_sd,   tau_sd / 2),
    sigma_v     = c(nu_sd * 0.3, nu_sd)
  )
...
derive_priors_from_pilot <- function(J_per, true, I_pilot = 20) {
  ...
  for (i in seq_len(I_pilot)) {
    for (k in seq_len(K)) {
      # mean_nu_ik is now log-scale mean for the lognormal
      ...
      # log-normal draw and no rejection loop needed
...
run_one <- function(I, J_per, true, model,
  ...
  K          <- 2
  cond_dummy <- c(0, 1)
  prs <- derive_priors_from_pilot(J_per, true, I_pilot = 20)
  x      <- rnorm(I, 0, 1)
  true_v <- rnorm(I, 0, true$sigma_v)
  X_nu    <- array(NA_real_, dim = c(I, K, 3))
  X_alpha <- array(NA_real_, dim = c(I, K, 2))
  ...
  for (i in seq_len(I)) {
    for (k in seq_len(K)) {
      # mean_nu_ik is the log-scale mean 
      mean_nu_ik    <- true$mu_nu + true$b1_nu * x[i] + true$b2_nu * cond_dummy[k]
      mean_alpha_ik <- true$mu_alpha + (true$b_alpha + true_v[i]) * cond_dummy[k]
      mean_tau_ik   <- true$mu_tau + true$b_tau * cond_dummy[k]
      repeat { a <- rnorm(1, mean_alpha_ik, true$sigma_alpha)
               if (a > 0.1 && a < 3.0) { alpha_true[i, k] <- a; break } }
      # log-normal draw and no rejection loop 
...
clusterExport(cl, varlist = c("run_one", "derive_priors_from_pilot",
                              "ez_point_estimates", "derive_priors", "true"))
clusterEvalQ(cl, {
  library(rstan)
  library(dplyr)
  rstan_options(auto_write = FALSE)
...
save_plot <- function(plot, filename, width = 8, height = 6) {
  ggsave(file.path("output", filename), plot, width = width, height = height,
         dpi = 300, bg = "#1e1e1e")
}
...
```
```{r version with only once making priors}
library(rstan)
library(bayesplot)
library(ggplot2)
library(dplyr)
...
true <- list(
  mu_nu    = 0.0,
  b1_nu    = 0.3,    # log-scale covariate effect: 1 SD in x multiplies drift by exp(0.3)
  b2_nu    = -0.5,   # log-scale condition effect: condition 2 drift = exp(-0.5) × condition 1
  mu_alpha = 1.5,
  b_alpha  = 0.2,
  mu_tau   = 0.3,
...
derive_priors <- function(ez) {
  ...
  log_nu_vals <- log(nu_vals)                              # work on log scale for nu
  nu_sd    <- min(max(sd(log_nu_vals), 0.3), 1.5)         # SD on log scale, bounded
  alpha_sd <- min(max(sd(alpha_vals),  0.2), 1.0)
  tau_sd   <- min(max(sd(tau_vals),    0.1), 0.5)
...
derive_priors_from_pilot <- function(J_per, true, I_pilot = 20) {
  ...
  for (i in seq_len(I_pilot)) {
    for (k in seq_len(K)) {
      ...
      repeat { a <- rnorm(1, mean_alpha_ik, true$sigma_alpha)
               if (a > 0.1 && a < 3.0) { ap <- a; break } }
      np <- exp(rnorm(1, mean_nu_ik, true$sigma_nu))       # log-normal draw
      repeat { t <- rnorm(1, mean_tau_ik, true$sigma_tau)
               if (t > 0.05) { tp <- t; break } }
      e      <- exp(-ap * np)
      pi_c   <- 1 / (1 + e)
...
run_one <- function(I, J_per, true, model, priors,
  ...
  K          <- 2
  cond_dummy <- c(0, 1)
  prs <- priors                                            # use pre-computed prior
  x      <- rnorm(I, 0, 1)
  true_v <- rnorm(I, 0, true$sigma_v)
  X_nu    <- array(NA_real_, dim = c(I, K, 3))
  X_alpha <- array(NA_real_, dim = c(I, K, 2))
  ...
  for (i in seq_len(I)) {
    for (k in seq_len(K)) {
      ...
      repeat { a <- rnorm(1, mean_alpha_ik, true$sigma_alpha)
               if (a > 0.1 && a < 3.0) { alpha_true[i, k] <- a; break } }
      nu_true[i, k] <- exp(rnorm(1, mean_nu_ik, true$sigma_nu))  # log-normal draw
      repeat { t <- rnorm(1, mean_tau_ik, true$sigma_tau)
               if (t > 0.05) { tau_true[i, k] <- t; break } }
    }
  }
...
# derive priors once per J value — 3 pilots total, never reused across main data
set.seed(42)
...
clusterExport(cl, varlist = c("run_one", "derive_priors_from_pilot",
                              ...
                              "true", "priors_by_J"))          # priors_by_J exported once
clusterEvalQ(cl, {
  library(rstan)
  library(dplyr)
  rstan_options(auto_write = FALSE)
...
results_list <- parLapply(cl, jobs_list, function(row) {
  run_one(I      = row$I,
          ...
          true   = true,
          model  = ez_cond,
          priors = priors_by_J[[as.character(row$J)]])         # look up by J value
})
stopCluster(cl)
saveRDS(results_list, file = "output/simulation_results.rds")
