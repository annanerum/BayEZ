# 6: alpha=cond, nu=cond+random slope on nu, tau=intercept
...
if (!dir.exists("output_model6")) dir.create("output_model6")
...
derive_priors_from_pilot <- function(J_per, true, I_pilot=20) {
  ...
  for (i in seq_len(I_pilot)) for (k in seq_len(K)) {
    mean_nu_ik    <- true$mu_nu + (true$b2_nu + true_v_p[i]) * cond_dummy[k]
  ...
  derive_priors(ez, P_nu=2, P_alpha=2, P_tau=1, has_sigma_v=TRUE)
}
...
run_one <- function(I, J_per, true, model, priors,
  ...
  for (i in seq_len(I)) for (k in seq_len(K)) {
    mean_nu_ik    <- true$mu_nu + (true$b2_nu + true_v[i]) * cond_dummy[k]
    mean_alpha_ik <- true$mu_alpha + true$b_alpha * cond_dummy[k]
    mean_tau_ik   <- true$mu_tau
  ...
  group_results <- data.frame(
    I = I, J = J_per,
    param    = c("mu_nu", "b2_nu", "mu_alpha", "b_alpha", "mu_tau",
                 "sigma_nu", "sigma_alpha", "sigma_tau", "sigma_v"),
    true_val = c(true$mu_nu, true$b2_nu, true$mu_alpha, true$b_alpha, true$mu_tau,
                 true$sigma_nu, true$sigma_alpha, true$sigma_tau, true$sigma_v),
    estimate  = s[c("beta_nu[1]", "beta_nu[2]", "beta_alpha[1]", "beta_alpha[2]",
                    "beta_tau[1]", "sigma_nu", "sigma_alpha", "sigma_tau", "sigma_v"), "mean"],
    converged = converged,
    rhat_max  = rhat_max,
  ...
  mean_ik_results <- data.frame(
    ...
    true_mean_nu   = as.vector(matrix(
      sapply(seq_len(K), function(k)
        rep(true$mu_nu + true$b2_nu * cond_dummy[k], I)), nrow=I)),
    true_mean_alpha = as.vector(matrix(
      sapply(seq_len(K), function(k)
        rep(true$mu_alpha + true$b_alpha * cond_dummy[k], I)), nrow=I)),
...
## compile Stan model
stan_file <- "model6_cond_alpha_cond_nu_slope_nu.stan"
...
priors_by_J <- setNames(
  lapply(J_vals, function(j) derive_priors_from_pilot(j, true_m6, I_pilot=20)),
  as.character(J_vals)
)
...
clusterExport(cl, varlist=c(
  ...
  "true_m6", "priors_by_J", "stan_file", "K"
))
...
clusterEvalQ(cl, {
  ...
  options(mc.cores=1)
  stan_model6 <<- stan_model(stan_file)
})
...
stopCluster(cl)
saveRDS(results_list, "output_model6/simulation_results.rds")
...
save_plot <- function(p, filename, width=10, height=8) {
  ggsave(file.path("output_model6", filename), p,
...
p_alpha <- ggplot(results_indiv, aes(x=true_alpha, y=est_alpha, colour=condition)) +
  ...
  labs(title="M6: alpha=cond, nu=cond+slope | alpha", colour="Condition") +
...
p_nu <- ggplot(results_indiv, aes(x=true_nu, y=est_nu, colour=condition)) +
  ...
  labs(title="M6: alpha=cond, nu=cond+slope | nu", colour="Condition") +
  dark_theme + theme(legend.position="bottom")
...
p_tau <- ggplot(results_indiv, aes(x=true_tau, y=est_tau, colour=condition)) +
  ...
  scale_colour_manual(values=c("Condition 1"="goldenrod","Condition 2"="steelblue")) +
  facet_grid(I_label ~ J_label) +
  labs(title="M6: alpha=cond, nu=cond+slope | tau", colour="Condition") +
  dark_theme + theme(legend.position="bottom")
...
save_plot(p_alpha, "m6_recovery_alpha.png")
save_plot(p_nu,    "m6_recovery_nu.png")
save_plot(p_tau,   "m6_recovery_tau.png")
save_plot(p_v,     "m6_recovery_v.png")

cat("\nDone. Results saved to output_model6/\n")