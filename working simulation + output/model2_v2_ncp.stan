// model: nu=cond+slope, alpha=cov, tau=intercept
// v[i] = beta_nu[2] + sigma_v * z_v[i]
// v[i] is the participant's total condition effect on nu
// centred at the population mean beta_nu[2], not 0
// shrinkage pulls toward beta_nu[2]

data {
  int I;
  int K;
  int P_nu;     // 2: intercept + condition mean
  int P_alpha;  // 2: intercept + covariate
...
parameters {
  // beta_nu[1] = population mean nu intercept
  // beta_nu[2] = population mean condition effect (prior mean for v[i])
  vector[P_nu]    beta_nu;
...
transformed parameters {
  // v[i] is the participant's total condition effect on nu
  // centred at beta_nu[2] (population mean), SD = sigma_v
  vector[I] v = beta_nu[2] + sigma_v * z_v;
}
...
model {
  ...
  for (i in 1:I) {
    for (k in 1:K) {
      // v[i] is now the slope on nu, beta_nu[2] absorbed into v[i]
      real mu_nu_ik    = beta_nu[1] + v[i] * X_nu[i, k][2];
...
generated quantities {
  ...
  for (i in 1:I) {
    for (k in 1:K) {
      mean_nu_ik[i, k]    = beta_nu[1] + v[i] * X_nu[i, k][2];
